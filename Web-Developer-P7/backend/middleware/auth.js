...
module.exports = (req, res, next) => { // vérifie si le token envoyé est valable et si l'user id correspond bien
  try {
    ...
    const decryptedToken = jwt.verify(token, process.env.TOKEN);  // fonction verify pour décoder le token (si il n'est pas valide une erreur sera génégée)
    const userId = decryptedToken.userId; // extrait de l'userID du token
